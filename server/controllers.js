...
//let queryText= `select json_agg(questions) as results from(select id as question_id, body as question_body, date_written as question_date, asker_name, helpful as question_helpfulness, reported from questions where product_id = $1 and reported = false) as questions limit $2 offset $3`
// let queryText = `select id as question_id, body as question_body, date_written as question_date, asker_name, helpful as question_helpfulness, reported from questions where product_id = $1 and reported = false order by question_id limit $2 offset $3`
  // let queryText = `select id, body, date_written, asker_name, helpful, reported from questions where product_id = $1 and reported = false order by id limit $2 offset $3`
  let query = {
    text: queryText,
      ...
      response.results = results.rows
       ...
       for(let i = 0; i < response.results.length; i++) {
        for(let answer_id in response.results[i].answers) {
          ...
          //response.results[i].answers.answer_id.photos = []
          let query = {
            // text: 'select url from answers_photos where answer_id = $1',
            text: `select coalesce(array_agg(url), '{}') as photos from answers_photos where answer_id = $1`,
            // text: `select coalesce(array_agg(url) photos from answers_photos filter (where answer_id = $1), '{}')`,
            // text: `select coalesce(select array_agg(url) photos filter (where url is not null), '{}') from answers_photos where answer_id = $1`,
            values: [answer_id]
          }
          let photo_urls = await pool.query(query)
          // console.log('PHOTO URLS', photo_urls.rows[0].photos)
